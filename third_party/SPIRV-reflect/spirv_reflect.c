...
#define CHECKED_READU32(parser, word_offset, value)                                              \
  {                                                                                              \
    ...
    if (checked_readu32_result != SPV_REFLECT_RESULT_SUCCESS) { return checked_readu32_result; } \
  }
...
static SpvReflectDecorationFlags ApplyDecorations(const Decorations* p_decoration_fields) {
  ...
  if (p_decoration_fields->is_buffer_block) { decorations |= SPV_REFLECT_DECORATION_BUFFER_BLOCK; }
  if (p_decoration_fields->is_row_major) { decorations |= SPV_REFLECT_DECORATION_ROW_MAJOR; }
  if (p_decoration_fields->is_column_major) { decorations |= SPV_REFLECT_DECORATION_COLUMN_MAJOR; }
  ...
  if (p_decoration_fields->is_non_writable) { decorations |= SPV_REFLECT_DECORATION_NON_WRITABLE; }
  return decorations;
}
...
static void
ApplyArrayTraits(const SpvReflectTypeDescription* p_type, SpvReflectArrayTraits* p_array_traits) {
  ...
  while (spirv_word_index < p_parser->spirv_word_count) {
    ...
    case SpvOpAccessChain: {
      ...
      if (p_access_chain->index_count > 0) {
        ...
        if (IsNull(p_access_chain->indexes)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
...
ParseFunction(Parser* p_parser, Node* p_func_node, Function* p_func, size_t first_label_index) {
  ...
  if (p_func->accessed_ptr_count > 0) {
    ...
    if (IsNull(p_func->accessed_ptrs)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
  }
...
static SpvReflectResult ParseMemberCounts(Parser* p_parser) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    for (size_t i = 0; i < p_parser->node_count; ++i) {
      Node* p_node = &(p_parser->nodes[i]);
      if ((p_node->op != SpvOpMemberName) && (p_node->op != SpvOpMemberDecorate)) { continue; }
    ...
    for (uint32_t i = 0; i < p_parser->node_count; ++i) {
      ...
      if (IsNull(p_node->member_decorations)) { return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED; }
    }
  }
  return SPV_REFLECT_RESULT_SUCCESS;
}
...
static SpvReflectResult ParseDecorations(Parser* p_parser) {
  for (uint32_t i = 0; i < p_parser->node_count; ++i) {
    ...
    CHECKED_READU32(p_parser, p_node->word_offset + 1, target_id);
    Node* p_target_node = FindNode(p_parser, target_id);
    if (IsNull(p_target_node)) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE; }
    // Get decorations
...
static SpvReflectResult
ParseDescriptorBindings(Parser* p_parser, SpvReflectShaderModule* p_module) {
...
static SpvReflectResult ParseDescriptorType(SpvReflectShaderModule* p_module) {
  ...
  for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count;
    ...
    case SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
      p_descriptor->resource_type = (SpvReflectResourceType)(
          SPV_REFLECT_RESOURCE_FLAG_SAMPLER | SPV_REFLECT_RESOURCE_FLAG_SRV);
      break;
...
static SpvReflectResult ParseUAVCounterBindings(SpvReflectShaderModule* p_module) {
  ...
  for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count;
    ...
    if (p_descriptor->descriptor_type != SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER) { continue; }
    ...
    if (p_counter_descriptor != NULL) { p_descriptor->uav_counter_binding = p_counter_descriptor; }
  }
...
static SpvReflectResult ParseDescriptorBlockVariableUsage(
  ...
  case SpvOpTypeStruct: {
    ...
    if (p_var->member_count == 0) { return SPV_REFLECT_RESULT_ERROR_SPIRV_UNEXPECTED_BLOCK_DATA; }
...
static SpvReflectResult
ParseFormat(const SpvReflectTypeDescription* p_type, SpvReflectFormat* p_format) {
  ...
  if (p_type->type_flags & SPV_REFLECT_TYPE_FLAG_VECTOR) {
    ...
    if (p_type->type_flags & SPV_REFLECT_TYPE_FLAG_FLOAT) {
      ...
      case 64: {
        ...
        case 3:
          *p_format =
              signedness ? SPV_REFLECT_FORMAT_R64G64B64_SINT : SPV_REFLECT_FORMAT_R64G64B64_UINT;
          break;
        case 4:
...
static SpvReflectResult ParseStaticallyUsedResources(
  ...
  for (size_t i = 0, j = 0; i < called_function_count; ++i) {
    ...
    memcpy(
        &used_variables[used_variable_count],
...
static SpvReflectResult ParseExecutionModes(Parser* p_parser, SpvReflectShaderModule* p_module) {
  ...
  if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
    for (size_t node_idx = 0; node_idx < p_parser->node_count; ++node_idx) {
      ...
      if (IsNull(p_entry_point)) { return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ENTRY_POINT; }
...
static SpvReflectResult
ParsePushConstantBlocks(Parser* p_parser, SpvReflectShaderModule* p_module) {
  for (size_t i = 0; i < p_parser->node_count; ++i) {
    Node* p_node = &(p_parser->nodes[i]);
    ...
    for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j) {
      ...
      for (uint32_t k = 0; k < p_set->binding_count; ++k) {
        bool found = SearchSortedUint32(
            ...
            p_entry->used_uniform_count,
            p_set->bindings[k]->spirv_id);
        if (found) { p_entry_set->bindings[p_entry_set->binding_count++] = p_set->bindings[k]; }
      }
    }
  }
